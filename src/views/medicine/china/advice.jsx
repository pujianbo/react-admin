...
export default class datalist extends Component {
    ...
    this.columns = [
      {
        ...
        key: 'id',
        render: (value, record) => {
          return <Link title='详情' to={`/medicine/china/detail/${record.id}/5`}><Icon type="exclamation-circle-o"/></Link>
        }
      }
    ];
  }
  ...
  ajaxFile(file, fileList) {
    ...
    uploadFile({
      url: `/v1/import/exceltohealthcaresuggest`,
      data
    }, (res) => {
      if (res.code == 0) {
  ...
  saveExcel() {
    ...
    ajaxBlob({
      url: '/v1/import/exportdrug',
      type: 'GET',
      filename: `中医自诊-养生建议.xls`
    }, () => {
  ...
  render() {
    ...
    return (<div>
      <Form className='frmbtntop text-right'>
        ...
        <Button href='#/medicine/china/advice/edit'>添加</Button>
      ...
      <Form layout="inline" className='frminput'>
        <Row gutter={8}>
          <Col span={8}>
            <FormItem label="养生标题">
              <Input placeholder='搜索养生标题' onChange={this.getValue.bind(this, 'name')} style={{
                  width: '220px'
                }}/>
            </FormItem>
          </Col>
      ...
      <Modal visible={visible} title='批量添加' onCancel={this.handleCancel.bind(this)} footer={null}>
        <FormItem {...formItemLayout} label="下载模板">
          <Button href={excelUrl + '/access/%E5%85%BB%E7%94%9F%E5%BB%BA%E8%AE%AE%E6%A8%A1%E6%9D%BF.xlsx'}>下载模板</Button>
